package com.wiryadev.shared.utils.mapper
...
interface DataMapper<DTO, ViewParam> :
    ViewParamMapper<DTO, ViewParam>, DataObjectMapper<DTO, ViewParam>
...
class ListMapper<DTO, ViewParam>(private val mapper: Mapper<DTO, ViewParam>) {
    fun toDataObjects(viewParams: List<ViewParam>?): List<DTO> {
        if (mapper !is DataObjectMapper) {
            throw IllegalStateException("Needs mapper DataObjectMapper.kt")
        }
    ...
    fun toViewParams(dataObjects: List<DTO>?): List<ViewParam> {
        if (mapper !is ViewParamMapper) {
            throw IllegalStateException("Needs mapper ViewParamMapper.kt")
        }
